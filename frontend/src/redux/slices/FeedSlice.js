...
export const loadPosts = createAsyncThunk('feed/loadPosts', async () => {
  const response = await axios.get('https://https://talk-to-beavs.herokuapp.com/api/feed/get_posts');
  return response.data.posts;
});

export const createPost = createAsyncThunk('feed/createPost', async (post, { rejectWithValue }) => {
  try {
    const response = await axios.post('https://https://talk-to-beavs.herokuapp.com/api/feed/create_post', post);
